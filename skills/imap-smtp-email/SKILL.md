...
### forward
Forward an email by UID to one or more recipients. Automatically copies the original subject (prepended with "Fwd: "), body (plain text and HTML), and all attachments. An optional prepend text can be added before the quoted original.

```bash
node scripts/imap.js [--account <name>] forward <uid> --to <email> [options]
```

**Required:**
- `<uid>`: The UID of the email to forward
...
- `--prepend-text <text>`: Text to prepend before the forwarded message
- `--from <email>`: Override sender address
- `--mailbox <name>`: Source mailbox (default: INBOX)

**Examples:**
```bash
# Simple forward
node scripts/imap.js forward 1234 --to recipient@example.com

# Forward with a note and CC
node scripts/imap.js forward 1234 --to colleague@example.com --cc manager@example.com --prepend-text "Please review this."

# Forward with multiple recipients
node scripts/imap.js forward 1234 --to "a@example.com,b@example.com" --prepend-text "FYI"
```
