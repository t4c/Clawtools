...
// Forward an email by UID to one or more recipients
async function forwardEmail(uid, mailbox, options) {
  // 1. Fetch the original email with all attachments
  const imap = await connect();
  let parsed;
  ...
  try {
    ...
    const item = messages[0];
    parsed = await simpleParser(item.body);
  } finally {
    imap.end();
  }

  // 2. Build the forwarded subject
  const originalSubject = parsed.subject || '(no subject)';
  const fwdSubject = originalSubject.toLowerCase().startsWith('fwd:')
    ? originalSubject
    : `Fwd: ${originalSubject}`;

  // 3. Build prepend text (optional message from the forwarder)
  const prependText = options['prepend-text'] || options.text || null;

  // 4. Build the forwarded body (prepend + quoted original)
  let textBody = '';
  let htmlBody = null;

  if (prependText) {
    textBody += prependText + '\n\n';
  }

  // Append quoted original text
  const origFrom = parsed.from?.text || 'Unknown';
  const origDate = parsed.date ? parsed.date.toISOString() : 'Unknown';
  const origTo = parsed.to?.text || 'Unknown';
  const origSubjectLine = parsed.subject || '(no subject)';

  textBody += `-------- Forwarded Message --------\n`;
  textBody += `From: ${origFrom}\n`;
  textBody += `Date: ${origDate}\n`;
  textBody += `Subject: ${origSubjectLine}\n`;
  textBody += `To: ${origTo}\n\n`;
  textBody += parsed.text || '';

  // Build HTML variant if original had HTML
  if (parsed.html) {
    let htmlPrepend = '';
    if (prependText) {
      const escaped = prependText.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>');
      htmlPrepend = `<p>${escaped}</p>`;
    }
    htmlBody =
      htmlPrepend +
      `<br><br><hr>` +
      ...
      `<b>Subject:</b> ${origSubjectLine}<br>` +
      `<b>To:</b> ${origTo}<br>` +
      `</div><br>` +
      parsed.html;
  }

  // 5. Build attachments from the original email
  const attachments = [];
  if (parsed.attachments && parsed.attachments.length > 0) {
...
async function main() {
  ...
  try {
    ...
    switch (command) {
      ...
      case 'forward':
        if (!positional[0]) {
          throw new Error('UID required: node imap.js forward <uid> --to <email> [--cc <email>] [--prepend-text <text>]');
        }
        if (!options.to) {
          throw new Error('Recipient required: node imap.js forward <uid> --to <email>');
        }
      ...
      default:
        ...
        console.error('Available commands: check, fetch, download, search, mark-read, mark-unread, list-mailboxes, list-accounts, forward');
        process.exit(1);
    }
